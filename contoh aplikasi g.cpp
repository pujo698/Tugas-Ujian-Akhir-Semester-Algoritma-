...
int countConnectedComponents(const vector<Node>& graph) {
    int numComponents = 0;

    // Reset flag visited untuk semua simpul
    for (const Node& node : graph) {
        const_cast<Node&>(node).visited = false;
    }
...
int main() {
    ...
    for (int i = 0; i < numEdges; ++i) {
        ...
        graph[source].edges.push_back({destination});
        graph[destination].edges.push_back({source});  // Graf tidak berarah
    }
